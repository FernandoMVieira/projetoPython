...
from tkinter import Tk, Label, StringVar, Entry, Button, Frame, Menu, Scrollbar, Toplevel, NO, TOP, X, Y, W, SOLID, \
    LEFT, RIGHT, BOTTOM, HORIZONTAL, VERTICAL

root = Tk()
root.title("PROJETO AV2")
width = 500
height = 500
...
janela_novo = None
caminho_banco = 'banco.AV2'


def database():
    conn = sqlite3.connect(caminho_banco)
    cursor = conn.cursor()
    query = '''CREATE TABLE IF NOT EXISTS lutadores (
    ...
    cursor.execute('SELECT * FROM lutadores ORDER BY nome')
...
def enviar_dados():
    if nome.get() == '' or idade.get() == '' or peso.get() == '' or categoria.get() == '' or pais.get() == '':
        ...
        cursor.execute(query,
                       (str(nome.get()), str(telefone.get()), str(idade.get()), str(email.get()), str(endereco.get())))
        conn.commit()
...
def selecionado(event):
    ...
    telefone.set('')
    telefone.set(itens_selecionados[2])
    idade.set('')
    idade.set(itens_selecionados[3])
    email.set('')
    ...
    # ------- LABELS - ATUALIZAR -------
    lbl_titulo = Label(form_titulo, text='Atulizando contatos', font=('arial', 18), bg='BLUE', width=200)
    ...
    ent_idade = Entry(form_contato, textvariable=idade, font=('arial', 12))
...
def inserir_dado():
    ...
    # ------- LABELS - INSERIR -------
    lbl_titulo = Label(form_titulo, text='Inserindo contatos', font=('arial', 18), bg='BLUE', width=200)
    lbl_titulo.pack(fill=X)
    lbl_nome = Label(form_contato, text='Nome', font=('arial', 12))
    lbl_nome.grid(row=0, sticky=W)
    lbl_telefone = Label(form_contato, text='Telefone', font=('arial', 12))
...
lbl_alterar = Label(bottom, text='🚑 Para alterar clique duas vezes no contato desejado!', font=('arial', 18),
                    width=500)
lbl_alterar.pack(fill=X)

# --------- BOTOES PRINCIPAL -------
bttn_incluir = Button(midleft, text='INSERER', bg='royal blue', command=inserir_dado)
bttn_incluir.pack()
